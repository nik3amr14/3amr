"""
app.py  v6.0 — Kurdish Sorani Subtitle Generator
TRUE PARALLEL: Both videos run simultaneously via background threads
bashdar77 / nik3amr14
"""
...
# ── FFmpeg ────────────────────────────────────────────────────────────────────
def _ffmpeg(cmd, out_msg_fn=None, label=""):
    try:
        r = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=7200)
        if r.returncode != 0:
            err = r.stderr.decode(errors="replace")[-600:]
            if out_msg_fn: out_msg_fn(f"❌ FFmpeg ({label}): {err}")
            return False
        return True
    except subprocess.TimeoutExpired:
        if out_msg_fn: out_msg_fn(f"⏱️ Timeout ({label})")
        return False
    except FileNotFoundError:
        if out_msg_fn: out_msg_fn("❌ FFmpeg نەدۆزرایەوە — packages.txt: ffmpeg")
        return False
    except Exception as e:
        if out_msg_fn: out_msg_fn(f"❌ {e}")
        return False
...
def _transcribe(audio_path, forced_lang, progress_fn=None):
    ...
    if not forced_lang:
        ...
        list(segs)
        lang = info.language
        if progress_fn: progress_fn(f"🌐 دۆزرایەوە: {info.language} ({info.language_probability:.0%})")
    ...
    if not rows:
        if progress_fn: progress_fn("⚠️ VAD هیچ نەدۆزی — بەبێ فیلتەر هەوڵ دەدەم...")
...
# ── ASS ───────────────────────────────────────────────────────────────────────
def _build_ass(trs, delay, font_sz, cr_font, anime, xltr, season, tech,
    ...
    h += f"Style: Kurdish,{fn},{font_sz},&H00FFFFFF,&H000000FF,&H00000000,&H50000000,-1,0,0,0,100,100,0,0,1,1.8,0.9,2,40,40,30,1\n"
    h += f"Style: Song,{fn},{font_sz},{sc},&H000000FF,&H00000000,&H50000000,-1,1,0,0,100,100,0,0,1,1.6,0.8,2,40,40,30,1\n"
    h += f"Style: Credit,{fn},{cr_font},&H00FFD700,&H000000FF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,1.5,1.0,2,30,30,15,1\n"
    h += f"Style: Watermark,{fn},{wm_sz},{wc},&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,1.2,0,{wm_al},15,15,15,1\n\n"
    ...
    for row in trs:
        ...
        evts.append(f"Dialogue: 0,{_ts(s)},{_ts(e)},{'Song' if _is_song(t) else 'Kurdish'},,0,0,0,,{t}")
    return h + "\n".join(evts)
...
# ══ BACKGROUND WORKER ════════════════════════════════════════════════════════
def _bg_worker(p: str, video_bytes: bytes, cfg: dict):
    ...
    try:
        ...
        # 4. Chunk + translate
        chunks = _build_chunks(rows, cfg["chunk_secs"])
        total  = len(chunks)
        trans  = []
        key_idx = 0

        for i, chunk in enumerate(chunks):
            _prog(f"🔄 بڕگەی {i+1} لە {total} ({int(i/total*100)}٪)")
        ...
        w["translation_done"] = True
        w["pct"]              = 100
        w["status"]           = "✅ وەرگێڕان تەواو بوو — ژێرنووسەکان دەستکاری بکە پاشان داگیران بکە"
        w["done"]             = True
...
def render_app(p: str):
    ...
    # ── Two columns: Settings | Main ─────────────────────────────────────────
    sc, mc = st.columns([1, 2], gap="large")

    with sc:
        st.markdown("#### 🔑 کلیلەکانی API (4 کلیل)")
        gkeys = []
        for i in range(1, 5):
            k = st.text_input(f"کلیل {i}", type="password", key=f"{p}_k{i}")
        ...
        lc          = st.selectbox("", list(LANGS.keys()), key=f"{p}_lang",
    ...
    with mc:
        ...
        with st.expander("ℹ️ کرێدیت", expanded=False):
            ...
            with rc1: cr_secs = st.number_input("⏱️ کات",1.0,30.0,4.0,0.5,key=f"{p}_crsec")
        ...
        with st.expander("🔖 لۆگۆ", expanded=False):
            ...
            with wa3: wm_pos = st.radio("ئەلا",["چەپ","ناوەڕاست","ڕاست"],index=2,
                                        key=f"{p}_wmpos",horizontal=False)
        ...
        running = w and not w.get("done", True)
        ...
        btn_resume = b2.button("⏭️ Resume",       key=f"{p}_resume",
                               ...
                               disabled=running or not w.get("translation_done"))
        btn_reset  = b3.button("🔄 ڕیسێت",        key=f"{p}_reset",
                               use_container_width=True, disabled=running)

        if btn_reset:
            _workers.pop(p, None)
            for k in [f"{p}_sid",f"{p}_burned",f"{p}_out_path",f"{p}_edits"]:
        ...
        if btn_start:
            if not uploaded:
                st.warning("⚠️ ڤیدیۆیەک بخەرە."); st.stop()
            if not gkeys:
                st.warning("⚠️ لانیکەم یەک کلیل بخەرە."); st.stop()
            ...
            t = threading.Thread(target=_bg_worker,
                                 args=(p, uploaded.read(), cfg), daemon=True)
            t.start()
            _workers[p]["thread"] = t
            st.rerun()
        ...
        if trs:
            ...
            # Use saved edits or rebuild from translations
        ...
        if w and w.get("translation_done") and w.get("translations"):
            ...
            if st.button("🔥 داگیراندن بۆ ناو ڤیدیۆ", key=f"{p}_burn",
                         use_container_width=True, type="primary"):
                if not uploaded:
                    st.warning("⚠️ ڤیدیۆکە دووبارە بخەرە."); st.stop()
                vid = TEMP_DIR / f"{sid}_input.mp4"
                if not vid.exists():
                ...
                ass_str = _build_ass(
                    trs        = w["translations"],
                    delay      = sub_delay,    font_sz    = font_sz,
                    cr_font    = cr_font,      anime      = anime_name,
                    xltr       = xltr,         season     = season_ep,
                    tech       = tech_line,    cr_secs    = cr_secs,
                    wm_text    = wm_text,      wm_sz      = wm_sz,
                    wm_col_hex = wm_col,       wm_al      = wm_al,
                    song_col_hex = song_color)
                ass_p = TEMP_DIR / f"{sid}_subs.ass"
                ass_p.write_text(ass_str, encoding="utf-8")
                out_p = TEMP_DIR / f"{sid}_subtitled.mp4"

                burn_errors = []
                with st.spinner("🔥 داگیراندن — ultrafast..."):
...
# ══ MAIN ═════════════════════════════════════════════════════════════════════
# Preload Whisper on main thread (with spinner)
if "wm_loaded" not in st.session_state:
    with st.spinner("🔊 Whisper large-v3-turbo بارکردن..."):
        _get_wm()
...
tab1, tab2 = st.tabs(["🎬  ئەپی یەکەم — ڤیدیۆی یەکەم",
                      "🎬  ئەپی دووەم — ڤیدیۆی دووەم"])
with tab1: render_app("t1")
with tab2: render_app("t2")

# ── Auto-refresh while any worker is running ──────────────────────────────────
