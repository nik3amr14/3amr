"""
app.py — Kurdish Sorani Cinematic Subtitle Generator
Dark Romance Theme | Gemini 6 Models | Bahij Janna | Song Toggle
Compatible: Streamlit Cloud · HuggingFace Spaces
Author: bashdar77 / nik3amr14  |  v4.0
"""

import os, re, sys, json, uuid, time, shutil, tempfile, subprocess
from pathlib import Path
from typing import Optional

import streamlit as st

_HERE = Path(__file__).parent.resolve()
if str(_HERE) not in sys.path:
    sys.path.insert(0, str(_HERE))

from faster_whisper import WhisperModel
from ai_translator import ai_translate, GEMINI_MODELS, THINKING_PRESETS

st.set_page_config(page_title='🎬 Kurdish Sorani Subtitle Generator',
                   page_icon='🎬', layout='wide')

_FONT_CANDIDATES = [
    _HERE / "Bahij Janna-Bold.ttf",
    _HERE / "Bahij_Janna-Bold_.ttf",
    _HERE.parent / "Bahij Janna-Bold.ttf",
    _HERE.parent / "Bahij_Janna-Bold_.ttf",
    Path("/app/Bahij Janna-Bold.ttf"),
    Path("/home/user/app/Bahij Janna-Bold.ttf"),
]
FONT_PATH: Optional[Path] = next((p for p in _FONT_CANDIDATES if p.exists()), None)

_CSS = '<style>\n/* ══ DARK ROMANCE THEME — Anime Boy with Red Roses ══════════════════ */\n:root{\n    --void:   #010007;\n    --dark:   #05020d;\n    --crim:   #7a0000;\n    --crim-h: #a81200;\n    --gold:   #8b6f35;\n    --gold-h: #c9a03e;\n    --sil:    #7070a0;\n    --gc:     rgba(110,0,0,.22);\n    --gp:     rgba(50,10,90,.18);\n}\n/* ── Main background: anime boy image + dark overlay ── */\n.stApp{\n    background-color:var(--void)!important;\n    background-image:\n        linear-gradient(to right,\n            rgba(1,0,7,.82) 0%,\n            rgba(3,0,10,.55) 45%,\n            rgba(1,0,7,.80) 100%),\n        linear-gradient(rgba(1,0,7,.30) 0%, rgba(1,0,7,.60) 100%),\n        url("data:image/jpeg;base64,");\n    background-size:cover;\n    background-position:center top;\n    background-attachment:fixed;\n}\n/* ── Gothic arch overlay ── */\n.stApp::before{\n    content:"";position:fixed;inset:0;pointer-events:none;z-index:0;\n    background-image:url("data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22100%22%20height%3D%22130%22%3E%3Cpath%20d%3D%22M14%2C130%20L14%2C65%20Q14%2C14%2050%2C3%20Q86%2C14%2086%2C65%20L86%2C130%22%20fill%3D%22none%22%20stroke%3D%22rgba%28100%2C0%2C0%2C0.05%29%22%20stroke-width%3D%221.3%22%2F%3E%3Cline%20x1%3D%2250%22%20y1%3D%223%22%20x2%3D%2250%22%20y2%3D%22130%22%20stroke%3D%22rgba%2880%2C0%2C0%2C0.028%29%22%20stroke-width%3D%220.6%22%2F%3E%3Cline%20x1%3D%2214%22%20y1%3D%2297%22%20x2%3D%2286%22%20y2%3D%2297%22%20stroke%3D%22rgba%2880%2C0%2C0%2C0.028%29%22%20stroke-width%3D%220.6%22%2F%3E%3C%2Fsvg%3E");\n    background-size:100px 130px;\n}\n/* ── Candlelight pulse ── */\n@keyframes pulse{0%,100%{opacity:1}40%{opacity:.96}70%{opacity:.98}}\n.stApp::after{\n    content:"";position:fixed;inset:0;pointer-events:none;z-index:0;\n    background:radial-gradient(ellipse at 30% 100%,rgba(140,40,0,.10) 0%,transparent 55%);\n    animation:pulse 5s ease-in-out infinite;\n}\n/* ── Sidebar ── */\nsection[data-testid="stSidebar"]{\n    background:linear-gradient(180deg,#030010 0%,#010008 100%)!important;\n    border-right:1px solid #1a0830!important;\n}\nsection[data-testid="stSidebar"] *{color:#ccc8ec!important;}\n/* ── Main block ── */\n.main .block-container{padding-top:1.4rem!important;max-width:1080px!important;}\n/* ── Headings ── */\nh1{color:#f0eeff!important;font-weight:900!important;\n    text-shadow:0 0 30px rgba(140,0,0,.55),0 0 70px rgba(60,0,0,.3),\n                0 2px 6px rgba(0,0,0,.9)!important;}\nh2,h3{color:#b8b0dc!important;font-weight:700!important;}\n/* ── Buttons ── */\n.stButton>button{\n    background:linear-gradient(135deg,#0a0520,#12082c)!important;\n    color:#c0b8ec!important;border:1px solid #200840!important;\n    border-radius:7px!important;font-weight:700!important;\n    letter-spacing:.4px!important;transition:all .18s!important;\n}\n.stButton>button:hover{\n    background:linear-gradient(135deg,#160830,#200a40)!important;\n    border-color:#5820a0!important;color:#fff!important;\n    box-shadow:0 0 18px var(--gp)!important;\n}\nbutton[data-testid="stBaseButton-primary"]{\n    background:linear-gradient(135deg,#5c0000,#7a0000)!important;\n    border-color:#bb1200!important;color:#fff!important;\n}\nbutton[data-testid="stBaseButton-primary"]:hover{\n    background:linear-gradient(135deg,#7a0000,#9a0000)!important;\n    box-shadow:0 0 28px var(--gc)!important;\n}\n/* ── Inputs ── */\n.stTextInput input,.stTextArea textarea,.stNumberInput input{\n    background:#070318!important;color:#d4d0f6!important;\n    border:1px solid #1c0c38!important;border-radius:7px!important;\n}\n.stTextInput input:focus,.stTextArea textarea:focus,.stNumberInput input:focus{\n    border-color:#5820a0!important;box-shadow:0 0 10px var(--gp)!important;\n}\n.stSelectbox>div>div{\n    background:#070318!important;border:1px solid #1c0c38!important;\n    border-radius:7px!important;color:#d4d0f6!important;\n}\n.stSlider .st-fy{background:var(--crim)!important;}\n.stProgress>div>div>div>div{\n    background:linear-gradient(90deg,#5c0000,var(--crim-h))!important;\n    border-radius:4px!important;\n}\n[data-testid="stFileUploaderDropzone"]{\n    background:#050218!important;border:2px dashed #1c0c38!important;\n    border-radius:10px!important;\n}\n[data-testid="stAlert"]{\n    background:#070318!important;border:1px solid #1c0c38!important;\n    border-radius:8px!important;\n}\n[data-testid="stExpander"] summary{\n    background:#070318!important;border:1px solid #1c0c38!important;\n    border-radius:8px!important;color:#c0b8e0!important;\n}\n[data-testid="stWidgetLabel"] p,.stTextInput label,.stSelectbox label,\n.stSlider label,.stNumberInput label,.stRadio label,.stTextArea label{\n    color:#8878c0!important;font-weight:600!important;\n}\nsmall,.stCaption{color:var(--sil)!important;}\nhr{border-color:#120622!important;}\n/* ── Toggle switch ── */\n.stToggle p{color:#c0b8ec!important;}\n/* ── Color picker ── */\n[data-testid="stColorPicker"]{background:#070318!important;}\n/* ── RTL Kurdish textarea ── */\n.stTextArea textarea{direction:rtl;text-align:right;font-size:.95rem;}\n</style>'

def _inject_css():
    st.markdown(_CSS, unsafe_allow_html=True)

def _hex_to_ass(h: str) -> str:
    """#RRGGBB  →  &H00BBGGRR  (ASS BGR format)"""
    h = h.lstrip("#")
    r, g, b = int(h[0:2],16), int(h[2:4],16), int(h[4:6],16)
    return f"&H00{b:02X}{g:02X}{r:02X}"

_MUSIC = set("♪♫🎵🎶♩♬")
def _is_song(text: str) -> bool:
    return any(c in text for c in _MUSIC)

def _init():
    for k, v in dict(transcript=None, translations=[], chunks=[],
                     cur_chunk=0, cur_gem_idx=0, final_video_path=None,
                     raw_ass="", is_running=False,
                     session_id=str(uuid.uuid4())[:8]).items():
        st.session_state.setdefault(k, v)

_inject_css()
_init()

with st.sidebar:
    st.title("⚙️ ڕێکخستنەکان")
    st.caption("کام سێرڤەر یەکەم جار وەرگێڕانەکە بکات؟")
    st.divider()

    # ── 5 API Keys ────────────────────────────────────────────────────
    st.subheader("🔑 کلیلەکانی API")
    st.markdown("**مۆدێلەکانی گووگڵ**")
    g1=st.text_input("کلیلی جێمینای ١",type="password",key="gk1")
    g2=st.text_input("کلیلی جێمینای ٢",type="password",key="gk2")
    g3=st.text_input("کلیلی جێمینای ٣",type="password",key="gk3")
    g4=st.text_input("کلیلی جێمینای ٤",type="password",key="gk4")
    g5=st.text_input("کلیلی جێمینای ٥",type="password",key="gk5")
    gemini_keys=[k for k in [g1,g2,g3,g4,g5] if k.strip()]
    st.divider()

    # ── 6 Models ──────────────────────────────────────────────────────
    st.markdown("**☯️ مۆدێلی Gemini**")
    selected_model=st.selectbox("mdl",GEMINI_MODELS,label_visibility="collapsed")

    # ── Thinking ──────────────────────────────────────────────────────
    st.markdown("**🧠 Thinking Mode**")
    thinking_label=st.selectbox("thk",list(THINKING_PRESETS.keys()),
                                index=3,label_visibility="collapsed")
    thinking_budget:Optional[int]=THINKING_PRESETS[thinking_label]
    st.divider()

    # ── Language ──────────────────────────────────────────────────────
    st.markdown("**🌐 زمانی ڤیدیۆکە**")
    LANGS={
        "Auto-Detect (خۆکار)":None,"Japanese (ژاپۆنی)":"ja",
        "English (ئینگلیزی)":"en","Persian (فارسی)":"fa",
        "Arabic (عەرەبی)":"ar","Spanish (ئیسپانیایی)":"es",
        "Hindi (هیندی)":"hi","Russian (ڕووسی)":"ru",
        "Chinese (چینی)":"zh","German (ئەڵمانی)":"de",
        "Italian (ئیتالی)":"it","Korean (کۆریایی)":"ko",
        "French (فرەنسی)":"fr","Turkish (تورکی)":"tr",
        "Portuguese (پورتوگالی)":"pt",
    }
    lang_choice=st.selectbox("lng",list(LANGS.keys()),label_visibility="collapsed")
    whisper_lang=LANGS[lang_choice]
    st.divider()

    # ── Song Toggle ───────────────────────────────────────────────────
    st.markdown("**🎵 ڕێکخستنی گۆرانی**")
    translate_songs=st.toggle(
        "وەرگێرانی گۆرانی بکات؟",
        value=True,
        help="ئەگەر چاڵاک بوو: گۆرانیەکە بۆ کوردی وەرگێردرێت. ئەگەر چاڵاک نەبوو: گۆرانیەکە وەک خۆی دەمێنێتەوە."
    )
    song_color=st.color_picker("🎨 رەنگی ژێرنووسی گۆرانی","#FFD700",
                               help="رەنگی ژێرنووسی گۆرانییەکان")
    st.divider()

    # ── Processing ────────────────────────────────────────────────────
    chunk_secs=st.slider("⏱️ قەبارەی بڕگە (خولەک)",3,15,6)*60
    font_size=st.slider("🔤 قەبارەی فۆنت",20,80,42)
    st.divider()
    st.caption("✅ فۆنت: "+FONT_PATH.name if FONT_PATH else "⚠️ Bahij Janna نەدۆزرایەوە")
    st.caption("🌹 Kurdish AI Subtitle Generator v4.0")

st.title("🎬 دروستکردنی ژێرنووسی کینەماتۆگرافیکی کوردی سۆرانی")
st.caption("تەنانەت ئەستێرەکانی ئاسمانیش هێندەی بریقەی چاوەکانت ڕووناک نین")

uploaded_file=st.file_uploader(
    "📁 ڤیدیۆی خۆت بخەرە",
    type=["mp4","mkv","avi","mov","webm","m4v","flv","ts","wmv"])
st.divider()

with st.expander("ℹ️ زانیاری ناساندنی دەستپێک",expanded=True):
    cc1,cc2=st.columns(2)
    with cc1:
        anime_name=st.text_input("🎬 ناوی فیلم / زنجیرە","")
        translator=st.text_input("✍️ ناوی وەرگێر","")
    with cc2:
        season_ep=st.text_input("📺 سیزن / ئەڵقە","")
        tech_line=st.text_input("💻 ناوی تەکنیک","Kurdish AI Subtitles")
    credit_secs=st.number_input("⏱️ کاتی کرێدیتەکان (چرکە)",
        min_value=1.0,max_value=30.0,value=3.0,step=0.5)

with st.expander("🎨 واتەرمارک",expanded=False):
    wc1,wc2=st.columns([3,1])
    with wc1:
        watermark_text=st.text_input("دەقی واتەرمارک (بەتاڵ = نەبێت)","")
    with wc2:
        wm_pos=st.selectbox("ئەلا",["سەرەوە-ڕاست","سەرەوە-چەپ"])

sub_delay=st.slider("⏰ شوێنکردنەوەی کاتی ژێرنووس (چرکە)",-15.0,15.0,0.0,0.1)
st.divider()

TEMP_DIR=Path(tempfile.gettempdir())/"kurdish_subs"
FONTS_DIR=TEMP_DIR/"fonts"
TEMP_DIR.mkdir(parents=True,exist_ok=True)
FONTS_DIR.mkdir(parents=True,exist_ok=True)
_FDEST=FONTS_DIR/"BahijJanna-Bold.ttf"
if FONT_PATH and FONT_PATH.exists() and not _FDEST.exists():
    shutil.copy2(FONT_PATH,_FDEST)

def _tp(s:str)->Path: return TEMP_DIR/f"{st.session_state.session_id}_{s}"

def _ffmpeg(cmd:list,label:str="")->bool:
    try:
        r=subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=7200)
        if r.returncode!=0:
            err=r.stderr.decode(errors='replace')[-900:]
            st.error(f"❌ FFmpeg ({label}):\n{err}")
            return False
        return True
    except subprocess.TimeoutExpired:
        st.error(f"⏱️ FFmpeg timeout ({label})")
        return False
    except FileNotFoundError:
        st.error("❌ FFmpeg نەدۆزرایەوە. packages.txt بیخەرە: ffmpeg")
        return False
    except Exception as exc:
        st.error(f"❌ {exc}"); return False

def extract_audio(video:Path)->Path:
    out=_tp("audio.wav")
    # dynaudnorm بۆ بەرزکردنەوەی دەنگی لاوازی دیالۆگ
    if _ffmpeg(["ffmpeg","-y","-i",str(video),"-vn",
               "-af","dynaudnorm=f=150:g=15:r=0.9",
               "-acodec","pcm_s16le","-ar","16000","-ac","1",str(out)],"extract_audio"):
        return out
    raise RuntimeError("دەرکردنی دەنگ سەرنەکەوت.")

@st.cache_resource(show_spinner="🔊 Whisper large-v3-turbo بارکردن...")
def _load_whisper()->WhisperModel:
    return WhisperModel("large-v3-turbo",device="auto",compute_type="int8")

def _segs_to_rows(segs)->list:
    rows=[]
    for seg in segs:
        txt=seg.text.strip()
        if not txt: continue
        if seg.words:
            s=round(seg.words[0].start,3)
            e=round(seg.words[-1].end,3)
        else:
            s=round(seg.start,3); e=round(seg.end,3)
        if e-s<0.4: e=s+0.4  # minimum duration
        rows.append({"start":s,"end":e,"text":txt})
    # fix overlaps
    for i in range(1,len(rows)):
        if rows[i]["start"]<rows[i-1]["end"]+0.04:
            rows[i]["start"]=rows[i-1]["end"]+0.04
    return rows

def transcribe_audio(audio:Path,forced_lang:Optional[str])->list:
    model=_load_whisper()
    lang=forced_lang
    # Pass 1: auto-detect language
    if not forced_lang:
        segs,info=model.transcribe(str(audio),beam_size=1)
        list(segs)
        lang=info.language
        st.info(f"🌐 زمانی دۆزراوە: **{info.language}** ({info.language_probability:.1%})")
    # Pass 2: main transcription WITH VAD
    segs,_=model.transcribe(str(audio),language=lang,
        vad_filter=True,vad_parameters={"min_silence_duration_ms":250},
        no_speech_threshold=0.20,word_timestamps=True,beam_size=3)
    rows=_segs_to_rows(segs)
    # Pass 3: fallback WITHOUT VAD if empty (catches quiet/muted videos)
    if not rows:
        st.warning("⚠️ VAD هیچ دەنگێک نەدۆزی — بەبێ فیلتەر دووبارە هەوڵ دەدەم...")
        segs,_=model.transcribe(str(audio),language=lang,
            vad_filter=False,no_speech_threshold=0.55,word_timestamps=True,beam_size=3)
        rows=_segs_to_rows(segs)
    return rows

def build_chunks(segs:list,max_secs:float)->list:
    if not segs: return []
    chunks,cur,t0=[],[],segs[0]["start"]
    for i,seg in enumerate(segs):
        cur.append(seg)
        if seg["end"]-t0>=max_secs:
            chunks.append(cur); cur=[]
            if i+1<len(segs): t0=segs[i+1]["start"]
    if cur: chunks.append(cur)
    return chunks

_KU=str.maketrans("0123456789","٠١٢٣٤٥٦٧٨٩")
def _ku(n:int)->str: return str(n).translate(_KU)

def _ts(sec:float)->str:
    sec=max(0.0,sec)
    h,m,s=int(sec//3600),int((sec%3600)//60),int(sec%60)
    return f"{h}:{m:02}:{s:02}.{int(round((sec-int(sec))*100)):02}"

def build_ass(translations,delay,font_sz,anime,xltr,season,tech,
              cr_secs,wm_text,wm_right,song_color_hex,translate_songs_flag)->str:
    wm_al=7 if wm_right else 8
    fn="Bahij Janna" if FONT_PATH else "Noto Naskh Arabic"
    song_ass_color=_hex_to_ass(song_color_hex)
    hdr=(
        "[Script Info]\nScriptType: v4.00+\n"
        "PlayResX: 1920\nPlayResY: 1080\n"
        "ScaledBorderAndShadow: yes\nYCbCr Matrix: TV.709\n\n"
        "[V4+ Styles]\n"
        "Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour,"
        " OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut,"
        " ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow,"
        " Alignment, MarginL, MarginR, MarginV, Encoding\n"
        f"Style: Kurdish,{fn},{font_sz},"
        "&H00FFFFFF,&H000000FF,&H00000000,&H64000000,"
        "-1,0,0,0,100,100,0,0,1,2.8,1.8,2,40,40,30,1\n"
        f"Style: Song,{fn},{font_sz},"
        f"{song_ass_color},&H000000FF,&H00000000,&H64000000,"
        "-1,1,0,0,100,100,0,0,1,2.5,1.5,2,40,40,30,1\n"
        f"Style: Credit,{fn},30,"
        "&H00FFD700,&H000000FF,&H00000000,&H96000000,"
        "-1,0,0,0,100,100,0,0,1,1.5,1,8,30,30,20,1\n"
        f"Style: Watermark,{fn},22,"
        "&H80FFFFFF,&H000000FF,&H00000000,&H00000000,"
        f"0,0,0,0,100,100,0,0,1,0,0,{wm_al},10,10,10,1\n\n"
        "[Events]\n"
        "Format: Layer, Start, End, Style, Name,"
        " MarginL, MarginR, MarginV, Effect, Text\n"
    )
    evts=[]
    parts=[p for p in [anime,f"وەرگێر: {xltr}" if xltr else "",season,tech] if p]
    if parts:
        evts.append(f"Dialogue: 1,0:00:02.00,{_ts(2+cr_secs)},Credit,,0,0,0,,{chr(92)+"N".join(parts)}")
    if wm_text.strip():
        last=(translations[-1].get("end",0)+delay) if translations else 3600.0
        evts.append(f"Dialogue: 0,0:00:00.00,{_ts(last)},Watermark,,0,0,0,,{wm_text}")
    for row in translations:
        s=row.get("start",0.0)+delay
        e=row.get("end",0.0)+delay
        t=row.get("text","").strip()
        if not t or s>=e: continue
        style="Song" if _is_song(t) else "Kurdish"
        evts.append(f"Dialogue: 0,{_ts(s)},{_ts(e)},{style},,0,0,0,,{t}")
    return hdr+"\n".join(evts)

def burn_subtitles(video:Path,ass:Path,out:Path)->bool:
    af=f"ass={str(ass)}"
    if _FDEST.exists(): af+=f":fontsdir={str(FONTS_DIR)}"
    return _ffmpeg(["ffmpeg","-y","-i",str(video),"-vf",af,
                   "-c:v","libx264","-preset","fast","-crf","18",
                   "-c:a","aac","-b:a","192k","-movflags","+faststart",
                   str(out)],"burn_subtitles")

b1,b2,b3=st.columns(3)
btn_start =b1.button("▶️ دەست پێبکە", use_container_width=True,type="primary")
btn_resume=b2.button("⏭️ Smart Resume",use_container_width=True)
btn_reset =b3.button("🔄 ڕیسێت",       use_container_width=True)

if btn_reset:
    for k in ["transcript","translations","chunks","cur_chunk",
              "cur_gem_idx","final_video_path","raw_ass","is_running","session_id"]:
        st.session_state.pop(k,None)
    st.rerun()

def _filter_for_ai(chunk:list,do_translate_songs:bool)->tuple:
    """Returns (to_translate, song_rows_passthrough).
    If translate_songs=False, song lines are kept as-is and not sent to AI."""
    to_ai,passthrough=[],[]
    for seg in chunk:
        if _is_song(seg["text"]) and not do_translate_songs:
            passthrough.append(seg)  # keep original text, mark as song
        else:
            to_ai.append(seg)
    return to_ai,passthrough

def run_pipeline(resume:bool=False):
    if not uploaded_file:
        st.warning("⚠️ تکایە ڤیدیۆیەک بخەرە."); return
    if not gemini_keys:
        st.warning("⚠️ تکایە لانیکەم یەک کلیلی Gemini بخەرە."); return

    vid=_tp("input.mp4")
    with open(vid,"wb") as f: f.write(uploaded_file.read())

    st.session_state.is_running=True
    status=st.empty(); pbar=st.progress(0)

    if not resume or st.session_state.transcript is None:
        status.info("🎵 دەرهێنانی دەنگ...")
        try: audio=extract_audio(vid)
        except RuntimeError as e: st.error(str(e)); st.session_state.is_running=False; return

        status.info("🔊 Whisper large-v3-turbo — گوێگرتن و خەمڵاندن...")
        try: transcript=transcribe_audio(audio,whisper_lang)
        except Exception as e: st.error(f"❌ Whisper: {e}"); st.session_state.is_running=False; return

        if not transcript:
            st.warning("⚠️ هیچ دەنگێک نەدۆزرایەوە."); st.session_state.is_running=False; return

        st.session_state.transcript=transcript
        st.session_state.chunks=build_chunks(transcript,chunk_secs)
        st.session_state.cur_chunk=0; st.session_state.translations=[]

    chunks=st.session_state.chunks
    total=len(chunks)
    start_i=st.session_state.cur_chunk if resume else 0
    trans=list(st.session_state.translations) if resume else []

    for i in range(start_i,total):
        pct=int(i/total*100)
        status.info(f"🔄 لە ٪{_ku(pct)} ی ڤیدیۆکە تەواو بووە... (بڕگەی {_ku(i+1)} لە {_ku(total)})")
        pbar.progress(pct)

        # Song toggle: filter chunk before sending to AI
        to_ai,passthrough=_filter_for_ai(chunks[i],translate_songs)

        ai_rows=[]
        if to_ai:
            ctx_b=chunks[i-1][-4:] if i>0 else []
            ctx_a=chunks[i+1][:4]  if i+1<total else []
            try:
                ai_rows,st.session_state.cur_gem_idx=ai_translate(
                    gemini_keys=gemini_keys,
                    cur_gem_idx=st.session_state.cur_gem_idx,
                    transcript_chunk=to_ai,
                    thinking_budget=thinking_budget,
                    selected_model=selected_model,
                    status_msg=status,
                    ctx_before=ctx_b, ctx_after=ctx_a,)
            except RuntimeError as e:
                st.error(f"❌ بڕگەی {i+1}: {e}"); st.session_state.is_running=False; return

        # Merge: add passthrough song rows back (keep original text, Song style)
        pt_rows=[{"start":s["start"],"end":s["end"],"text":s["text"]}
                 for s in passthrough]
        chunk_result=sorted(ai_rows+pt_rows,key=lambda x:x["start"])

        trans.extend(chunk_result)
        st.session_state.translations=trans
        st.session_state.cur_chunk=i+1

    pbar.progress(100)
    status.success("✅ وەرگێڕانی هەموو بڕگەکان تەواو بوو!")

    # Build ASS
    ass_str=build_ass(
        translations=st.session_state.translations,
        delay=sub_delay,font_sz=font_size,anime=anime_name,xltr=translator,
        season=season_ep,tech=tech_line,cr_secs=credit_secs,
        wm_text=watermark_text,wm_right=("ڕاست" in wm_pos),
        song_color_hex=song_color,translate_songs_flag=translate_songs,
    )
    st.session_state.raw_ass=ass_str
    ass_p=_tp("subs.ass")
    ass_p.write_text(ass_str,encoding="utf-8")

    # Burn
    status.info("🔥 داگیراندنی ژێرنووس بۆ ناو ڤیدیۆ...")
    out_p=_tp("subtitled.mp4")
    if burn_subtitles(vid,ass_p,out_p):
        st.session_state.final_video_path=str(out_p)
        status.success("🎉 ڤیدیۆی ئامادەی دانلۆد!")
    else:
        st.error("❌ داگیراندن سەرنەکەوت.")
    st.session_state.is_running=False

if btn_start:  run_pipeline(resume=False)
if btn_resume:
    if st.session_state.transcript is None:
        st.warning("⚠️ هیچ کۆدێکی پێشوو نییە — لە سەرەوە دەست پێبکە.")
    else: run_pipeline(resume=True)

if st.session_state.translations:
    st.divider()
    st.subheader("✏️ دەستکاریکردنی ژێرنووس")
    plain="\n".join(
        f'{r.get("start",0):.2f} --> {r.get("end",0):.2f} | {r.get("text","")}'
        for r in st.session_state.translations)
    edited=st.text_area("ژێرنووسەکانت لێرە دەستکاری بکە:",value=plain,height=320)
    if st.button("💾 جێبەجێکردنی گۆڕانکاریەکان"):
        updated=[]
        for line in edited.splitlines():
            m=re.match(r"(\d+\.?\d*)\s*-->\s*(\d+\.?\d*)\s*\|(.*)",line.strip())
            if m:
                updated.append({"start":float(m.group(1)),"end":float(m.group(2)),"text":m.group(3).strip()})
        if updated: st.session_state.translations=updated; st.success("✅ پاشەکەوتکرا.")
        else: st.error("❌ فۆرمات هەڵەیە.")

if st.session_state.final_video_path:
    out_p=Path(st.session_state.final_video_path)
    if out_p.exists():
        st.divider()
        st.subheader("⬇️ دانلۆدی ڤیدیۆی ئامادەکراو")
        mb=out_p.stat().st_size/1024/1024
        st.caption(f"📦 قەبارەی فایل: {mb:.1f} MB")
        with open(out_p,"rb") as fh:
            st.download_button(label="📥 دانلۆدی ڤیدیۆ (subtitled.mp4)",
                               data=fh,file_name="subtitled.mp4",mime="video/mp4",
                               use_container_width=True,type="primary")
        if mb<150: st.video(str(out_p))
        else: st.info("ℹ️ فایلەکە گەورەیە — دانلۆد بکە بۆ سەیرکردن.")
